//
//  TodayRedPacketListViewController.m
//  FullHelp
//
...
@interface TodayRedPacketListViewController ()
...
@implementation TodayRedPacketListViewController
- (instancetype) init {
    if (self = [super init]) {
        _pageIndex = 1;
        _pageSize = 30;
...
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:GetTodayRedpacketNotification object:nil];
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getTodayRedpacketNotification:) name:GetTodayRedpacketNotification object:nil];
...
- (void)getRedPacketList {
    [[[RedPacketAdvertNetWorkEngine alloc] init] getTodayListWithIndustryID:[IndustryInfo getIndustryInfo].industryID userID:[UserInfoEngine getUserInfo].userID page:_pageIndex pageSize:_pageSize successed:^(NSInteger code, NSMutableArray *arrData) {
        switch (code) {
            case 100:
            {
                if (_pageIndex == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell == nil) {
        cell = [[TodayCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:menuCell];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    RedPacketAdvertDetailViewController *redPacketAdvertDetailViewController = [[RedPacketAdvertDetailViewController alloc] initWithRedPacketAdvertID:redPacketInfo.redPacketAdvertID];
