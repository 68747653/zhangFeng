...
- (void)loadData {
    ...
    [self getRechargeRecordListWithUserID:[UserInfoEngine getUserInfo].userID page:_pageIndex pageSize:_pageSize];
}

- (void)getRechargeRecordListWithUserID:(NSInteger)userID page:(NSInteger)page pageSize:(NSInteger)pageSize {
    self.op = [[[AccountNetWorkEngine alloc] init] getRechargeRecordListWithUserID:userID page:page pageSize:pageSize successed:^(NSInteger code, NSMutableArray *arrData) {
            ...
            case 101: {
                ...
                if (!_isLoadMore && !_isPullToRefresh) {
                    ...
                    [self showLoadDataFailViewInView:self.view WithText:[GlobalFile HHSoftLoadNoDataMessage] failImage:[GlobalFile HHSoftLoadNoDataImage] failTouch:^{
                        ...
                        _isPullToRefresh = NO;
                        [self showLoadingViewWithText:[GlobalFile HHSoftLoadingWaitMessage] animationImages:[GlobalFile HHSoftLoadingAnimationImages] animationDuration:[GlobalFile HHSoftLoadingAnimationDuration]];
                        [self getRechargeRecordListWithUserID:userID page:page pageSize:pageSize];
                    }];
                } else {
...
- (HHSoftTableView *)dataTableView {
    if (!_dataTableView) {
        ...
        [_dataTableView addPullToRefresh:^{
            ...
            [_weakself getRechargeRecordListWithUserID:[UserInfoEngine getUserInfo].userID page:_weakself.pageIndex pageSize:_weakself.pageSize];
        }];
        //加载更多
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [RechargeRecordTableCell getCellHeightWith:accountChangeInfo];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    AccountChangeInfo *accountChangeInfo = _arrData[indexPath.row];
    static NSString *identifier = @"RechargeRecordTableCell";
    RechargeRecordTableCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[RechargeRecordTableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
