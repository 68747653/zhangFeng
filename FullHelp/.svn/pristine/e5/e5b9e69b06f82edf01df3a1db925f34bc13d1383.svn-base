...
@interface AttentionNewsViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic,copy) AttentionNewsCanEditBlock canEditBlock;
@property (nonatomic,copy) AttentionNewsIsAllDeleteBlock isAllDeleteBlock;
@property (nonatomic,strong) UIView *bottomView;
@property (nonatomic,assign) BOOL isAllSelect;

@end

@implementation AttentionNewsViewController
...
- (void)viewDidLoad {
    ...
    //获取我的关注资讯列表
    [self getCollectNewsListWithPageIndex:_pageIndex PageSize:_pageSize UserID:[UserInfoEngine getUserInfo].userID];
}
#pragma mark -- 重新刷新数据
...
#pragma mark -- 获取我的关注资讯列表
-(void)getCollectNewsListWithPageIndex:(NSInteger)pageIndex
                              PageSize:(NSInteger)pageSize
                                UserID:(NSInteger)userID{
    self.op = [[[AttentionNetWorkEngine alloc] init] getCollectNewsListWithPageIndex:pageIndex PageSize:pageSize UserID:userID Succeed:^(NSInteger code, NSMutableArray *arrNewsList) {
        ...
        if (code == 100) {
            ...
            [self.arrData addObjectsFromArray:arrNewsList];
            ...
            if (!_isLoadMore&&!_isPullToRefresh) {
                ...
                [self showLoadDataFailViewInView:self.view WithText:[GlobalFile HHSoftLoadNoDataMessage] failImage:[GlobalFile HHSoftLoadNoDataImage] failTouch:^{
                    ...
                    //调接口
                    //获取我的关注资讯列表
                    [self getCollectNewsListWithPageIndex:pageIndex PageSize:pageSize UserID:[UserInfoEngine getUserInfo].userID];
                }];
            }else {
...
-(HHSoftTableView *)dataTableView{
    if (_dataTableView == nil) {
        ...
        [_dataTableView addPullToRefresh:^{
            ...
            //调接口
            //获取我的关注资讯列表
            [_weakself getCollectNewsListWithPageIndex:_weakself.pageIndex PageSize:_weakself.pageSize UserID:[UserInfoEngine getUserInfo].userID];
        }];
...
-(UIView *)bottomView{
    if (_bottomView == nil) {
        ...
        chooseButton.tag = 742;
        [_bottomView addSubview:chooseButton];
        [chooseButton addTarget:self action:@selector(allChooseNewsButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        //删除
        ...
        [deleteButton addTarget:self action:@selector(deleteNewsButtonClick) forControlEvents:UIControlEventTouchUpInside];
    }
    //全选、取消全选
    UIButton *chooseButton = (UIButton *)[_bottomView viewWithTag:742];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    AttentionNewsViewCell *cell = [tableView dequeueReusableCellWithIdentifier:menuCell];
    if (cell == nil) {
        cell = [[AttentionNewsViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:menuCell];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.editing) {
        ...
        //类型【1：视频、2：图文, 3：图片新闻，4：外部链接】
        switch (attentionInfo.newsInfo.newsType) {
            case 1:{
                VideoNewsInfoViewController *videoNewsInfoViewController = [[VideoNewsInfoViewController alloc] initWithNewsID:attentionInfo.newsInfo.newsID newsImage:attentionInfo.newsInfo.newsImage infoID:0];
                [self.navigationController pushViewController:videoNewsInfoViewController animated:YES];
            }
                break;
            case 3:{
                PictureNewsInfoViewController *pictureNewsInfoViewController = [[PictureNewsInfoViewController alloc] initWithNewsID:attentionInfo.newsInfo.newsID infoID:0];
                [self.navigationController pushViewController:pictureNewsInfoViewController animated:YES];
            }
                break;
            case 2:
            case 4:{
                NewsInfoViewController *newsInfoViewController = [[NewsInfoViewController alloc] initWithUrl:attentionInfo.newsInfo.newsLinkUrl newsID:attentionInfo.newsInfo.newsID infoID:0];
...
-(void)allChooseNewsButtonClick:(UIButton *)button{
